...
def display_view_records_tab():
    ...
    # Use vehicle options from config, including "All"
    filter_vehicle = st.selectbox("Filter by Vehicle:", VEHICLE_OPTIONS + [
                                  "All"], index=len(VEHICLE_OPTIONS))  # "All" is the last option
    ...
    if latest_10_trips:
        ...
        st.dataframe(df_display[columns_to_display], hide_index=True)
    else:
